...
if ( !class_exists( 'PB_Update' ) ) {
	...
	class PB_Update {
		...
		public function __construct( $current_version, $plugin_slug, $license_user = '', $license_key = '' ) {
			...
			$hook = 'in_plugin_update_message-' . PB_PLUGIN_BASENAME;
		...
		public function check_update( $transient ) {
			...
			if ( version_compare( $this->current_version, $remote_version->new_version, '<' ) ) {
				...
				upgrade_notice = '2.7 is a major update. Make a full site backup with theme and extensions before upgrading.'
				tested = ''
				requires_php = ''
				compatibility = ''
				 */
		...
		function action__in_plugin_update_message( $plugin_info_array, $plugin_info_object ) {
			if ( empty( $plugin_info_array['package'] ) ) {
				echo ' Please <a href="' . esc_url( admin_url( 'admin.php?page="pb-license-activation"' ) ) . '">add your license</a> to update.';
			}
		}
	}
}
