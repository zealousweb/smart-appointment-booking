	...
	class PB_Front_Action {
		...
		*/
		...
		function zealbms_get_booking_form($attr) {
			...
			// Output the calendar
			?>
			<div style='display: inline-block; vertical-align: top;margin-left:660px'>
				<div class="month-navigation">
					<span class="arrow" id="prev-month">&larr;</span>
					<select name='bms_month' id='month'>
						<?php
						for ($i = 1; $i <= 12; $i++) {
							echo "<option value='$i'";
							if ($i == $currentMonth) {
					...
					while ($dayCounter <= $totalCells) {
						...
						for ($i = 1; $i <= 7; $i++) {
							if ($dayCounter >= $firstDayOfWeek && $date <= $totalDays) {
								$isToday = ($date == date('j') && $monthYear == date('n-Y')) ? "bmstoday" : "";
								echo "<td data_day='bms_" . $post_id . '_' . $currentMonth . "_" . $date . "_" . $currentYear . "' class='bms_cal_day $isToday' onclick='getClickedId(this)'>$date</td>";
								$date++;
